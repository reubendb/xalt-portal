...
try {
    ...
    $sql="SELECT xr.run_id AS RunId, xr.job_id AS JobId, xr.date AS Date,
        ...
        IF (
            ...
            where jro.run_id = xr.run_id) >= 1, 'true', 'false')
        AS ObjAtRunTime
        FROM xalt_run AS xr
        WHERE 
        xr.uuid = '$uuid' 
...
{\"id\":\"\",\"label\":\"Obj@RunTime[T/F]\",\"pattern\":\"\",\"type\":\"boolean\"} 
], 
\"rows\": [ ";
